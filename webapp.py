...
import datetime
from load_stocks_data import main


# Set the number of inter-operation threads
...
symbol = [symbol + ".NS" for symbol in symbol]
...
#company info
 ...
 # closing price chart
    ...
    cursor.execute("SELECT Distinct* from stock_price where symbols= (?) and Date >= (?) and Date <= (?)",
                   (ticker, start_input, end_input))
    hist_price = pd.DataFrame(cursor.fetchall(),
                          columns=['Date', 'Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume','Symbols'])
...
if (feature == 'Technical Indicators'):
    ...
    df_ma = calc_moving_average(df, 14).copy()
    ...
    df_boll = calc_bollinger(df, 20).copy()
    ...
    kama_df = df.copy()
    kama = ta.momentum.KAMAIndicator(kama_df['Close'],20,2,20)
    kama_df['kama'] = kama.kama()
    kama_data = kama_df.reset_index()
    fig_kama = go.Figure()
    fig_kama.add_trace(
        go.Scatter(
            x=kama_data['Date'],
    ...
    aroon_df = df.copy()
    aroon = ta.trend.AroonIndicator(aroon_df['Close'],aroon_df['Low'], window = 14)
    aroon_df['aroon_down'] = aroon.aroon_down()
    aroon_df['aroon_indicator'] = aroon.aroon_indicator()
    aroon_df['aroon_up'] = aroon.aroon_up()

    data_aroon = aroon_df.reset_index()
    fig_aroon = go.Figure()
    fig_aroon.add_trace(
        go.Scatter(
            x=data_aroon['Date'],
    ...
    trix_df = df.copy()
    trix = ta.trend.TRIXIndicator(trix_df['Close'], 14)
    trix_df['trix'] = trix.trix()

    data_trix = trix_df.reset_index()
    fig_trix = go.Figure()
    fig_trix.add_trace(
        go.Scatter(
            x=data_trix['Date'],
...
if (feature == 'Next-Day Forecasting'):
    ...
    # Function for next-day forecasting with LSTM
    def my_LSTM(ticker, cursor):
        try:
            history_range = 6
            today = datetime.datetime.now().date()
            hist_date = today - datetime.timedelta(days=history_range * 365)

            # Execute SQL query with date range filter
            cursor.execute("SELECT * FROM stock_price WHERE Symbols = ? AND Date >= ?", (ticker, hist_date))
            df = pd.DataFrame(cursor.fetchall(),
                            columns=['Date', 'Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume', 'symbols'])
            ...
            model.fit(x_train, y_train, epochs=9, batch_size=36, verbose=2)
            ...
            go.Scatter(
                x=train['Date'],
                y=train['Adj Close'],
                mode='lines',  # Use 'lines' mode to show only lines without markers
                line=dict(width=3),
            ...
            st.write('#### Metrics')
            st.write('Absolute error between predicted and actual value:', round(mae, 2))
            st.write('Root mean squared error between predicted and actual value:', round(rmse, 2))
            ...
            with st.container(height=200):
                ...
                col_111.metric(f'Closing Price Prediction of \n The next trading day {str(today + datetime.timedelta(days=1))} \n For {ticker} is', f' ₹ {str(round(float(prediction[0][0]), 2))}')

        except:
            st.warning("Oops! you can't go ahead!!")
            st.warning("The company you selected is listed newly...so we can't gather data.")

    my_LSTM(ticker, cursor)
...
#             with st.container(height=200):
#                 col_111, col_222, col_333 = st.columns(3)
#                 col_111.metric(f'Closing Price Prediction of \n The next trading day {str(today + dt.timedelta(days=1))} \n For {ticker} is',f' ₹ {str(round(float(prediction[0][0]),2))}')
...
if (feature == 'Pattern Recognition'):
    ...
    end_input = dt.datetime.today() + dt.timedelta(days=1)
    cursor.execute("SELECT * from stock_price where symbols= (?) and Date > (?) and Date < (?)",
    ...
    st.table(signal_df.style.map(color_survived, subset=['Signal']))
...
#         co_1.metric('EBITDA Margin', info.get('ebitdaMargins',''))
#         co_2.metric('Profit Margin', info.get('profitMargins',''))
#         co_3.metric('Gross Margin', info.get('grossMargins',''))
#         co_4.metric('Operating Margin', info.get('operatingMargins',''))

#     with st.container():
#         co_11, co_22,co_33,co_44 = st.columns(4)
#         co_11.metric('Current Ratio', info.get('currentRatio',''))
#         co_22.metric('Return on Assets', info.get('returnOnAssets',''))
#         co_33.metric('Debt to Equity', info.get('debtToEquity',''))
#         co_44.metric('Return on Equity', info.get('returnOnEquity',''))
...
if (feature == "Screener"):
    ...
    stock = yf.Ticker(ticker_input)
    info = stock.info
    closing_price = round((df['Close'].iloc[-1:]).values[0], 2)
    opening_price = round(df['Open'].iloc[-1:].values[0], 2).astype('str')
    sma_df = calc_moving_average(df, 12)
    sma_df_tail = round(float(sma_df['sma'].iloc[-1]), 2)
    ema_df_tail = round(float(sma_df['ema'].iloc[-1]), 2)

    macd_df = calc_macd(df)
    ema26_df_tail = round(float(macd_df['ema26'].iloc[-1]), 2)
    macd_df_tail = round(float(macd_df['macd'].iloc[-1]), 2)
    signal_df_tail = round(float(macd_df['signal'].iloc[-1]), 2)

    rsi_df = RSI(df, 14)
    rsi_df_tail = round(float(rsi_df['RSI'].iloc[-1]), 2)

    adx_df = ADX(df, 14)
    adx_df_tail = round(float(adx_df.iloc[-1]), 2)
